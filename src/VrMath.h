...
// 3x3 or 3x4 matrix
template <class T> vr::HmdQuaternion_t HmdQuaternion_FromMatrix(const T& matrix) {
...
static vr::HmdQuaternion_t HmdQuaternion_FromSwingTwist(const vr::HmdVector2_t& swing, const float twist) {
    ...
    if (swing_squared > 0.f) {
        ...
        result.y = (swing.v[1] * cos_half_twist * sin_half_theta_over_theta)
                 - (swing.v[0] * sin_half_twist * sin_half_theta_over_theta);
        result.z = (swing.v[0] * cos_half_twist * sin_half_theta_over_theta)
                 + (swing.v[1] * sin_half_twist * sin_half_theta_over_theta);
    }
...
template <class T, class Q> void HmdQuaternion_ConvertQuaternion(const T& in_quaternion, Q& out_quaternion) {
...
template <class T, class V> void HmdVector3_CovertVector(const T& in_vector, V& out_vector) {
